# Tweet-Native AI Harness 设计概览

## 1. 产品定位

这个产品不是“套了一层 Twitter 外观的聊天界面”，而是一个短消息驱动的工作系统：用户发布轻量任务请求，Agent 通过线程化、带状态的回帖来执行工作。

最新结构回到以 `Thread` 为最上层：

1. 用户在看板中点击“发布任务 / 新建线程”。
2. 用户为线程选择本地文件夹、设定线程名称、选择任务 Role，并写下不超过 500 字的任务请求。
...
500 字限制是有意设计，不是表达能力不足。它迫使每条消息都成为一个聚焦的小决策包，而不是脆弱又臃肿的长规格说明。
...
自动模式是对人工拖入 WIP 的补位机制：开启后后端每 5 分钟检查当前 Ready 栏，当 WIP 少于 5 个线程时，从 Ready 栏启动 queued run，直到 WIP 补满或 Ready 栏为空。自动模式不直接扫描 Backlog 全量。后台 worker 默认最多并发执行 5 个 Codex run，并发上限通过 `CODEX_WORKER_CONCURRENCY` 配置但固定封顶为 5。
...
这套设计最稳的理解方式不是“一个像 Twitter 的 AI 产品”，而是“一套短消息工作协议”：
