...
自动模式开启时，后端立即检查一次 Ready 栏快照，之后按持久化设置中的 1 到 10 分钟间隔循环检查，默认间隔为 5 分钟。当 WIP 线程数低于设置中的并发上限时，系统从当前 Ready 栏中选取可启动的 queued run 补入 WIP，直到达到上限或 Ready 为空。这里的输入边界是 Ready 栏快照，不是 Backlog 全量；同一轮检查不会因为 Backlog 中下一批任务被自动显示而继续消费整个 Backlog。worker 支持 1 到 5 个 Codex run 并发执行，默认并发为 5，固定上限为 5。
