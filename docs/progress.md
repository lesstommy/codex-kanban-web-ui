...
- 自动模式已接入：开启后每 5 分钟检查 Ready 栏，当 WIP 少于 5 个线程时自动把 Ready 栏中的 queued run 补入 WIP；不会在同一轮扫描 Backlog 全量
- worker 支持最多 5 个 Codex run 并发执行，默认 `CODEX_WORKER_CONCURRENCY=5`，并在服务启动时恢复 WIP 中仍为 queued 的 run
...
- 测试真实调用 Codex app-server，不保留 fake runner
...
- 用户回复当前先创建同一 task 下的新 run；后续应更精确接入 Codex `turn/steer` 或 app-server thread resume
